...
import CreditReceiptModal from './CreditReceiptModal';

type NavigationProp = NativeStackNavigationProp<RootStackParamList>;

interface CreditCodeModalProps {
  visible: boolean;
  onClose: () => void;
  phone: string;
  amount: string;
...
export default function CreditCodeModal({
  visible,
  onClose,
  phone,
  amount,
    ...
    if (code.length === 4) {
      if (code.join('') === '1234') {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            setShowLoader(false);
            Alert.alert('Achat de crédit réussi avec succès');
            setShowReceipt(true);
          }, 2000);
        }, 300);
        setCode([]);
